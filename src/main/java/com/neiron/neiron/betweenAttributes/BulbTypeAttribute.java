...
public class BulbTypeAttribute {
    public final static HashMap<String, Integer> dependencies = new HashMap<>();

    static {
       dependencies.put("connectorType=4", 27);//g13
        dependencies.put("connectorType=12", 29);//r7s
//        dependencies.put("bulbType=13", 3);

    }
    public static Item findBulbTypeByAnotherAttributes(Item item) {
        if(item.getBulbType()==null){
        item.setBulbType(useDependencies(item));}
        return item;
    }
